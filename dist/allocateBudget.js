...
// discussion.
function allocateBudget(riskRecords, budget = catalog_1.WEEKLY_REORDER_BUDGET) {
    ...
    const ranked = [...atRiskNeedingReorder].sort((a, b) => {
        ...
        if (daysA !== daysB)
            return daysA - daysB; // soonest stockout first
        if (a.salesVelocityPerDay !== b.salesVelocityPerDay)
            return b.salesVelocityPerDay - a.salesVelocityPerDay; // faster mover first
    ...
    for (const record of ranked) {
        if (remaining <= 0) {
            funded.push(toRecommendation(record, 0, 'Not funded this week: budget already exhausted by higher-priority (more urgent) SKUs.'));
            continue;
        }
        if (record.idealReorderCost <= remaining) {
            ...
            if (partialQty > 0) {
                funded.push(toRecommendation(record, partialQty, `Partially funded: remaining budget covered ${partialQty} of the ideal ${record.idealReorderQuantity} units.`));
    ...
    const passthrough = everythingElse.map((record) => toRecommendation(record, 0, record.atRisk
...
function toRecommendation(record, fundedQty, note) {
    return {
        ...record,
