...
test('funds the SKU with the highest value-per-rupee first, not the most urgent one', () => {
  const records = [
  ...
  const result = allocateBudgetByValue(records, 100000);
  ...
  assert.equal(lowValue.recommendReorder, true);
  assert.equal(lowValue.reorderQuantity, 13);
  assert.equal(highValue.recommendReorder, true);
  ...
  assert.ok(highValue.reorderQuantity < 500);
  assert.ok(highValue.reorderQuantity > 0);
});

test('never allocates more than the given budget', () => {
  const records = [
  ...
  const result = allocateBudgetByValue(records, 20000);
  const total = result.reduce((sum, r) => sum + r.reorderCost, 0);

  assert.ok(total <= 20000);
});

test('SKUs that are not at risk pass through with zero value at risk and no reorder', () => {
  ...
  assert.equal(result[0].valueAtRisk, 0);
});
